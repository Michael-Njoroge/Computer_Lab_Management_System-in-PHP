...
if(isset($_SESSION['Id'])) {
	...
	if(empty($email) && empty($password)){
		...
		$sql = "SELECT * FROM tbltechnician WHERE emailAddress = '$email'";
		$query = $conn->query($sql);

		if($query->num_rows == 1) {
			$password = md5($password);

			$mainSql = "SELECT * FROM tbltechnician WHERE emailAddress = '$email' AND password = '$password'";
			$mainResult = $conn->query($mainSql);

			if($mainResult->num_rows == 1) {
				$value = $mainResult->fetch_assoc();
				$user_id = $value['technician_id'];
			...
			<div class="col-md-12">
				...
				<div class="box box-widget widget-user">
					...
					<div class="widget-user-header bg-aqua-active">
						<h3 class="widget-user-username text-center">Lab Technician Panel</h3>

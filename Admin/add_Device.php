...
$dev_serial = substr(str_shuffle($set), 0, 4);
 									
						
$query = mysqli_query($conn,"select * from tbldevice where dev_serial = '$dev_serial' ");
...
$insert=mysqli_query($conn,"insert into tbldevice (dev_id,dev_desc,dev_serial,dev_brand,dev_model,dev_status) values('$dev_id','$dev_desc','$dev_serial','$dev_brand','$dev_model','$dev_status')");
 
$note=mysqli_query($conn,"insert into tblactivity_log (date,username,action) values(NOW(),'$admin_username','Add device Detail id $dev_id')");
